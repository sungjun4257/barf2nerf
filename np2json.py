...
############# Please change and check the File Path ###################

file_path="./llff.json"               #json file path
np_path="./barf_output_pose.npy"      #numpy file(barf output) path
img_path="./llff/images"              #dataset image file path

#############################################################

img_list=sorted(os.listdir(img_path)) # load image name list

path=['./llff/images/']               # make image_path like json file
...
np_data=np.load(np_path)            # load numpy file
print(np_data.shape)
np_data=np_data.tolist()            # numpy to list
 

list_homo=[0.0 , 0.0 , 0.0 , 1.0]   # make homogeneous matrix
...
for change_num in range(0,len(img_list)):   # change original json file data
   for i in data["frames"]:   
      if i["file_path"] == img_list[change_num]:
